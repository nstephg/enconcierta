<nav class="md:hidden fixed bottom-0 left-0 right-0 z-[9999] bg-[#0C0C13] border-t border-white/10 px-3 pt-2 pb-[max(0.75rem,env(safe-area-inset-bottom))] flex items-center justify-around shadow-[0_-10px_30px_rgba(0,0,0,0.95)]">
    <!-- Feed -->
    ...
    <!-- Botón Crear Post Flotante -->
    <button type="button" onclick="openCreatePostModal()" class="flex items-center justify-center w-11 h-11 rounded-full bg-[#FF3D57] text-white shadow-[0_0_16px_rgba(255,61,87,0.6)] active:scale-95 transition-transform -mt-5 border-2 border-[#0A0A0F] cursor-pointer">
